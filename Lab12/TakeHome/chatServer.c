...
// Function to store the chat history in a file. Avoid storing duplicate chat history.
void storeChatHistoryInFile() {
    FILE *file;
    char filename[MESSAGE_SIZE];
    bzero(filename, MESSAGE_SIZE);
    sprintf(filename, CHAT_LOG_FILE);
    ...
    for (int i = 0; i < SERVER_BACKLOG; i++) {
        if (chatHistoriesList[i]->isValid == 1) {
            for (int j = 0; j < CHAT_HISTORY_SIZE; j++) {
                if (chatHistoriesList[i]->ch[j]->sourceClientId != -1) {
...
int handleClientRequest(int clientSocket, char *request) {
    ...
    if (strcmp(command, "/active") == 0) {
        ...
        // storing thechat history in a file
        storeChatHistoryInFile();
